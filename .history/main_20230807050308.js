...
function startSorting() {
  ...
  document.getElementById('sortingForm').classList.remove('text-center');
}
...
function sortStudent(e) {
  ...
  if (studentName === '') {
    nameError.classList.remove('text-center');
    return;
  }
...
function expelStudent(e) {
    e.preventDefault();
    const expelledCadet =  students.splice(e, 1)[0];
    console.log(expelledCadet)
    expelledCadet.push({name: studentName, ship: expelShip.title, message: expelShip.message, image: expelShip.imageUrl});
...
document.getElementById('studentsContainer').addEventListener('click', function (e) {
  if (e.target.classList.contains('btn-expel')) {
    ...
    // const studentIndex = starship.findIndex(i => i.id === Number(id))
