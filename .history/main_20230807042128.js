...
function expelStudent(studentIndex) {
    ...
    expelledCadet.push({name: expelledStudent, ship: expelShip.title, message: expelShip.message, image: expelShip.imageUrl, isExpelled: true});
...
document.getElementById('studentsContainer').addEventListener('click', function (e) {
  if (e.target.classList.contains('btn-expel')) {
    const studentIndex = e.target.value('.col-md-3').dataset.index;
