...
function renderToDom(containerId, shuffleShips) {
  const container = document.getElementById(containerId);
  container.innerHTML = '';
  shuffleShips.forEach((item) => {
    const card = `
      <div class="col-md-3 mb-4">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">${item.name}</h5>
            <p class="card-text">Ship: ${item.ships.title}</p>
            <p class="card-text">Message: ${item.ships.message}</p>
